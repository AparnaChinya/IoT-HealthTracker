...
class AzureIoTHub
{
    ...
    //
    // Note: this connection string is specific to the device "newIOTDevice". To configure other devices,
    // see information on iothub-explorer at http://aka.ms/iothubgetstartedVSCS
    //
    const string deviceConnectionString = "HostName=iOT-UCI.azure-devices.net;DeviceId=newIOTDevice;SharedAccessKey=H+SHRcM10Q3cYR63+CmuPmFtf8ig4TKZApTZU7puGPE=";


    //
    // To monitor messages sent to device "kraaa" use iothub-explorer as follows:
    //    iothub-explorer monitor-events --login HostName=iOT-UCI.azure-devices.net;SharedAccessKeyName=service;SharedAccessKey=v/6nOxwZWDnMSvjqdbl4csLOfSGjatAoBlwbNYxRrvM= "newIOTDevice"
    //
    ...
    public static async Task SendDeviceToCloudMessageAsync()
    {
        CreateClient();
#if WINDOWS_UWP
        var str = "{\"deviceId\":\"newIOTDevice\",\"messageId\":1,\"text\":\"Hello, Cloud from a UWP C# app!\"}";
#else
        var str = "{\"deviceId\":\"newIOTDevice\",\"messageId\":1,\"text\":\"Hello, Cloud from a C app!\"}";
#endif
      //  var message = new Message(Encoding.ASCII.GetBytes(str));

        await deviceClient.SendEventAsync(new Message(Encoding.ASCII.GetBytes(str)));
    }
